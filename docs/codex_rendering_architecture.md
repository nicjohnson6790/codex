...
DisplayTransformRenderer owns only its pipeline, sampler and camera exposure (default 1.0). It computes 1 − exp(−radiance × exposure), then the exact piecewise sRGB transfer (0.0031308 breakpoint, 12.92 linear slope, 1.055 × x^(1/2.4) − 0.055 shoulder). Blending is disabled and alpha is always one. This is the only ordinary display conversion; there is no per-medium exposure or automatic adaptation.
...
- kSpaceRadiance remains 0.0001 on decoded cubemap radiance, independently of the environment scale.
