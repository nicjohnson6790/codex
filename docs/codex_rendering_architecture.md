...
Fullscreen clouds intersect the prepared 5×5 field rectangle and vertical slab, then clip to foreground depth; water shares those intersections but retains its own 180 km limit and uniform sample counts. The full-domain view uses max(48, ceil(interval length/2000 m)) samples capped by a tunable 512 budget, with quadratic boundaries length × (0.25u + 0.75u²). It traverses the complete loaded interval with finer near-entry spacing. At the longest diagonal, default gaps reach about 12.7 km: macro coverage is sampled more densely than its lattice, but thin detail can alias. Worst-case default work is 3,584 density evaluations per pixel before early exits. GPU timings and distant detail acceptance remain pending; see [the exposure/cloud handoff](atmosphere_exposure_handoff.md).
