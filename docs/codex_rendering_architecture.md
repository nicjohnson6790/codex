...
Settings are exposed in Controls / Clouds, with texture dimensions configured at startup. `cloud_tests` compiles actual GLSL noise/shape/optical/slab math on CPU and checks periodicity, cache churn, borders, seed invalidation, universe-scale phases, directional stretch/travel, phase wrapping/freezing/subdivision, coverage multiplication, top monotonicity/legacy equivalence, slab intersections, scattering limits, and analytic integration. `--verify-clouds --quit-after-frames 720 --disable-steam` exercises GPU traversal across both slab boundaries, horizontal rays, grid transitions, seed changes, and cells at 2^55. The default marcher is deliberately straightforward: quality/cost are controlled by view and sun sample counts. The app configure step applies `cmake/PatchSDLVulkan.cmake` to pinned SDL 3.4.0, using VK_REMAINING_ARRAY_LAYERS only for 3D image barriers. This preserves whole-volume synchronization with or without maintenance9 while retaining single-layer array/cubemap barriers. It removes the compatibility warning without filtering validation or enabling a Vulkan feature. The guarded, idempotent patch handles already-populated sources and must be reviewed when upgrading SDL.
