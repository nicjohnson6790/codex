...
Terrain bodies and bridges now sample cloud shadows in their shared fragment shader. `CloudRenderer::samplingResources()` exposes the full prepared state independently of water budgets; terrain borrows the coverage/noise bindings and copies the 128-byte density field plus extinction/termination/readiness/sample count into a 144-byte fragment block at slot 1. Both terrain pipelines declare two fragment uniform blocks and nine samplers (cloud coverage/noise at 7/8). The existing 336-byte terrain block uses its formerly reserved vector at offset 96 for sine-elevation endpoints. No cloud shadow texture or additional pass is allocated.
...
Terrain solar visibility smoothly rises from zero at the horizon to one at +1 degree; direct diffuse/specular and already-masked caustics share one visibility-times-cloud-transmission factor. The march skips absent solar visibility/intensity/color, but does not use N dot L alone because specular and caustics can contribute independently. Ambient retains its albedo, solar tint, AO and 0.26 coefficient, modulated separately from zero at −6 degrees to unchanged at +6 degrees. Named defaults live in AppConfig::Terrain. These changes affect linear terrain radiance only; shared sunlight, water/foliage lighting and all exposure behavior remain unchanged.

Follow-up user review accepted exposure limits −8/+6 EV but found weak sun/shadow contrast and dull sunlit snow. A trial terrain sunlight multiplier improved contrast but was rejected as the wrong model and removed with its UI. At default intensity and near-normal incidence, diffuse is approximately 0.41 × albedo versus up to 0.26 × albedo ambient, limiting opaque-shadow contrast to roughly 2.6:1 before display mapping (ignoring specular/AO/atmosphere). The next lighting design should replace the fixed ambient floor with sky/cloud-dependent diffuse illumination. That replacement is not yet implemented. Diffuse sky illumination must remain distinct from the directional solar ray: blocking the sun does not block the whole sky. Auto exposure, surface orientation and display mapping also affect snow brightness.
