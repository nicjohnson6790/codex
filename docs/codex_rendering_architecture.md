...
Canopy lookup is a fixed four-candidate approximation requested by the user after the expanded-footprint implementation caused concern about GPU cost. Derive floor(patchMeters / 4 - 0.5) and its +X/+Z neighbors directly, without clamping at cell borders; the halo slot and seed resolve the owning cell. Compute stores occupancy bits, not a nearest-candidate list. Each accepted bit reconstructs the canonical class, jitter and rotation for local capture sampling. Directly index halo arrays in the SSBO, reject fades and losing seeds before expensive work, and sample the winning normal once. Capture gradients are derived before discard. CPU sky receiver lookup uses half-open tile domains.

The reported 1–2 FPS regression remains unconfirmed in the affected view. The fixed four-candidate path passed Debug/Release builds, seven tests each, canopy SPIR-V validation, 280 edge/corner mapping cases and a 120-frame Debug startup run with no logged errors. The preceding expanded-search old/new comparison at startup did not reproduce the severe slowdown; process durations include initialization and are not GPU timings. Reproduce the user's camera and measure foliage pass cost before declaring recovery.

### 5.3 Nearby detailed foliage and decoded-page cache

Schema 2 validation: Debug/Release/Assets Release builds, seven tests per runtime configuration, seven affected SPIR-V modules, production pine conversion/validation, and a 120-frame Debug run passed. The regenerated pack is staged into both builds with matching hashes. Decoding 40 normal images across four pitch bands and their mip chains confirmed both Z signs survive compression; all 16 capture records use schema 2 and BC3 normal textures. These checks do not establish visual elimination of shading bands.

Grazing-view correction: canopy cutout uses explicit base-level alpha independently of filtered RGB and normals; no view-angle cull exists. Existing edge/residency fades remain. The subsequent normal-band fix supersedes the view-basis reconstruction and adjacent-yaw correction: schema 2 captures normals in tree-local space and applies only instance rotation at runtime. The old filter discarded signed Z and reconstructed the camera-facing hemisphere, introducing capture-angle-dependent bias. Full signed XYZ is now filtered before linear BC3 compression; this retains the same block byte size as BC5. Different capture elevations still see different surfaces, so identical shading between views is not guaranteed.
...
### 5.4 Registered foliage captures and shared illumination

Nearby meshes switch at horizontal camera distances 5 and 25 m. LOD 2 remains admitted through 105 m, with its existing 100–105 m dither fade; imposters begin at 100 m. Both passes share geometry and alpha rejection. Imported tree-local X/Z and Y=0 ground are the placement reference for all representations. Nearby LODs are no longer independently recentered; their CPU culling centers use the same instance rotation as the shaders.

The explicit pine conversion emits 32 imposter layers (eight yaw views at camera elevations 0/25/50/75 degrees, pitch-major order) and three 64-square canopy color/normal layers per class. Positive elevation places the capture camera above the target. Imposters retain 1024-to-512 supersampling; canopy uses a 128-square viewport/readback in the existing reusable targets, downsampled to 64. The existing capture source is the lowest-numbered non-billboard LOD. Canopy views are cumulative geometry at local Y <= 10, Y <= 20, and the complete tree, without caps. Runtime shells remain at 10/20/30 m; the final capture camera is above the actual tree, independent of shell height.

A 64-byte version-1 FoliageCaptureRecord extension stores the tree-local capture center, common padded horizontal extent, four projected half-heights, common canopy X/Z framing, and two canopy texture references. AssetRecord's previously unused word at offset 20 is now captureMetadataOffset; zero identifies assets without this extension. Base asset/mesh/texture schemas remain v4, while the capture schema explicitly rejects old pine packs and unknown capture versions. Non-foliage packs need no regeneration. Bounds must be finite, extents positive, and the complete rotated footprint plus jitter must fit within one neighboring 256 m cell. Regenerate pinetreepack after this change; there is no legacy imposter interpretation.

Each imposter pitch uses a yaw-independent cylindrical bounds envelope, so blended views share framing. Runtime billboards tilt continuously over the supported 0–75 degree range and blend adjacent capture pitches as requested by the user, superseding the original nearest-pitch requirement. The quad encloses both serialized pitch bounds; separate UVs map physical coordinates into each capture and outside-frame samples are transparent. A shared fragment helper gives color and depth exactly the same alpha-weighted two-yaw by two-pitch blend. Degenerate horizontal camera directions use a deterministic yaw. Schema 2 capture normals store full signed tree-local XYZ in linear BC3; runtime blends yaw/pitch samples in the same local frame and applies instance rotation once, without capture pitch/yaw reconstruction. Color blending is also alpha weighted. Canopy normals use the same signed tree-local encoding and canonical instance rotation.

Capture material alpha rejection uses the source cutoff and writes binary surviving coverage. RGB dilation leaves alpha untouched. Supersampling and mip RGB filtering operate on linear material color; mip filtering uses alpha weights. Coverage correction selects the nearest attainable coverage instead of always rounding upward. Each mip filters the unadjusted preceding mip, preventing repeated coverage scaling from accumulating. BC3 stores linear color/alpha (UNORM); hardware sRGB decoding happens once when the converter samples authored base color. No exposure or time-of-day lighting is baked into captures. Compression and yaw interpolation can still change fine subpixel gaps and require visual review.

Canopy shells sample the same hashed class, jitter, and quantized rotation as canonical foliage. Physical capture bounds replace circles, palette colors, synthetic normals, and radius/shade multipliers. Only the four surrounding lattice candidates are considered, as explicitly requested by the user; large captures can overlap from omitted candidates. This supersedes the ticket's requirement to include every supported footprint; overlapping surviving samples choose the greatest candidate seed, with stable iteration as a tie break. Draw metadata contains a fixed 10x10 cell table: the existing up-to-8x8 interior plus a ready-only halo. The manager validates semantic IDs and protects referenced slots; this adds no population, generation queue, or halo-driven admission. Unavailable neighboring data remains uncovered until resident. Existing shell topology, edge fades and residency fades remain unchanged. The descriptor is 864 bytes (cell slots at 64, seeds at 464).

All foliage paths now consume the existing sky/cloud SH irradiance and shared surfaceSunIrradiance. Diffuse uses albedo/pi with the already convolved probe field. Direct diffuse, specular and compatible leaf transmission share the terrain 0–1 degree horizon factor and pure Beer cloud transmission once. Sky irradiance is independent of that directional factor. Legacy ambient floors, daylight fill, global brightness corrections and synthetic canopy darkening are removed. Nearby foliage retains its linear specular sky path; exposure remains solely in the display transform.

FoliageLighting borrows the prepared cloud textures/state and two read-only probe buffers. Its fragment slot-1 block is 256 bytes: density at 0, shadow parameters at 128, optics at 144, atmosphere at 224, horizon at 240. Nearby uses 9 samplers and 3 storage buffers; imposters 4/2; canopy 4/5. Tree-class records are 64 bytes. Frame receiver hints are derived from the terrain-emitted regions after probe preparation and before foliage uploads; shaders reassociate positions crossing their hinted tile. Existing ancestor/fallback behavior and command-queue ordering are retained. Foliage does not schedule probe work or retain region indices between frames.

Converter performance inspection confirms reusable device/pipelines/targets and per-asset geometry/material resources. There is still one submit/fence wait per view; asynchronous pipelining is outside this workgroup. createMaterialTextureSet copies imported payloads and re-decodes source files, and shared material texture references are not deduplicated. RGB dilation's six scans cannot propagate beyond the first ring because alpha is deliberately unchanged, making repeated passes an inexpensive follow-up opportunity. Filtering/compression are synchronous CPU work; relative phase costs have not been measured. A full final conversion completed in about 152 seconds while builds were also running (an earlier iteration took 109 seconds); these totals are not an optimization comparison.

Validation: canonical Debug/Release and Assets Release builds passed, all seven tests passed in both runtime configurations, and all 83 runtime/converter SPIR-V modules validated. Compiled foliage bindings, 256-byte lighting block, 64-byte class records and 864-byte canopy records agree with C++ assertions. The pine pack was regenerated and validated through the production reader (16 assets, 134 textures); decoded canopy contact sheets show the cumulative clipping layers. A Release startup frame also passed. The 720-frame Debug traversal passed all 12 stages and 24 macro revisions with 1,755 sky-tile updates and no logged validation errors. This traversal covers integration and origin changes; it does not establish appearance or per-representation performance.

User appearance checks remain necessary: mesh/imposter registration through yaw and elevation changes, gaps at mip transitions, canopy borders/overlap, and agreement under daylight, low sun, night and moving clouds. Sparse ground probes remain an approximation over tree height. Compilation, pack validation and GPU validation do not establish visual acceptance or GPU cost.
...
| Water shallow/mid/deep, shore, blue styling, foam and optional LOD tint | Human-authored sRGB, decoded through authored_color.glsl before lighting and material interpolation. Alpha remains data. |
...
Terrain bodies and bridges now sample cloud shadows in their shared fragment shader. `CloudRenderer::samplingResources()` exposes the full prepared state independently of water budgets; terrain borrows the coverage/noise bindings. Its fragment block at slot 1 contains the 128-byte density field, extinction/termination/readiness/sample count at 128, shared atmosphere optics at 144 and atmosphere parameters at 224 (240 bytes total). Both terrain pipelines declare two fragment uniform blocks and nine samplers (cloud coverage/noise at 7/8). The existing 336-byte terrain block uses its formerly reserved vector at offset 96 for sine-elevation endpoints. No cloud shadow texture or additional pass is allocated.

Terrain fragment positions include sampled elevation relative to the active camera Position, which is also the cloud upload's render origin. The shadow ray intersects the shared finite footprint/slab domain and uniformly samples positive-interval midpoints, reusing `sampleCloudDensity` and all prepared phases. Pure Beer transmission uses the existing extinction in inverse meters and termination threshold; it returns accumulated transmission on termination. Disabled/unready clouds, zero extinction and empty intervals transmit one. The Clouds panel's independent Terrain cloud shadow samples setting defaults to 8 and is clamped to 1–32. Long grazing paths can undersample detail, and available coverage ends at the prepared footprint; no water distance cap, replacement coverage, jitter or temporal reconstruction is used.

Terrain solar visibility smoothly rises from zero at the horizon to one at +1 degree; direct diffuse/specular and already-masked caustics share one visibility-times-cloud-transmission factor. The march skips absent solar visibility/intensity/color, but does not use N dot L alone because specular and caustics can contribute independently. Diffuse sky illumination now comes from the probe field in section 19, replacing the earlier 0.26 ambient coefficient and −6/+6-degree approximation. The direct horizon default lives in AppConfig::Terrain. Shared sunlight and water/foliage lighting remain unchanged.

Follow-up user review accepted exposure limits −8/+6 EV but found weak sun/shadow contrast and dull sunlit snow. A trial terrain sunlight multiplier improved contrast but was rejected as the wrong model and removed with its UI. Fallback commit `0c66927` was pushed before implementing the sky-illumination field below. Diffuse sky illumination remains distinct from the directional solar ray: blocking the sun does not block the whole sky. Auto exposure, surface orientation and display mapping also affect snow brightness.

User visual/performance review for terrain lighting remains pending: compare clear daytime materials; sunset/deep-night/sunrise slopes and specular; moving directional/evolving clouds and disabled clouds; body/bridge boundaries and terrain inside/above the layer; shallow-water caustics with their existing wave/shoreline masks; and origin crossings, low sun and coverage edges. Use existing lighting/cloud controls and leave exposure unchanged for this review. GPU timing and the quality/cost of 8 midpoint samples need user evaluation; runtime validation does not establish appearance or performance.

Terrain lighting validation: canonical Debug/Release builds and all six tests in each pass. `cloud_tests` compiles the actual solar/ambient helpers and shadow march with a constant-density fixture, checking bounded continuous transitions, analytic Beer transmission, monotonic optical depth, disabled/zero/empty paths, inside/above/grazing rays and accumulated early termination. Existing field-phase and domain tests cover origin shifts and universe precision. All 78 staged SPIR-V modules across both configurations validate; terrain fragment resource layouts match across configurations and C++ assertions. The existing 720-frame Debug GPU traversal passed all 12 stages and 24 macro revisions without logged validation warnings/errors (`build/Debug/app/terrain-lighting-traversal.log`); a Release startup frame also exited successfully. These checks do not establish nighttime visual brightness, terrain shadow quality or GPU timing.
...
## 19. Terrain-following sky illumination probes

The user visually accepted the probe lighting and corrected direct-sun balance. Generated build-folder logs were cleaned up after acceptance; the validation results below are retained as the record. GPU timing remains outstanding; foliage integration is described in section 5.4.

Terrain direct sunlight uses surfaceSunIrradiance from atmosphere.glsl, sharing the solar RGB/intensity and existing atmosphere/cloud source scale × environment scale used by sky scattering. Air transmission is evaluated at the fragment's terrain altitude; cloud Beer transmission and the terrain horizon transition then apply once. The initial probe integration retained an unscaled legacy terrain sun, making cloud-derived irradiance disproportionately bright relative to direct light. Sharing the source calibration corrects that mismatch without an independent terrain multiplier or probe compensation. The visible sun disk retains its separate artistic calibration; this correction does not establish energy conservation for the artistic cloud multiple-scattering/ambient model. This correction passed Debug/Release builds, all six tests in each (including vacuum source calibration, night, altitude and scale response), validation of all 80 shaders, compiled uniform-offset inspection, and a 120-frame Debug GPU run with no logged validation errors. Visual sun/cloud balance remains pending user review.

SkyIlluminationManager owns semantic residency, scheduling and receiver metadata; SkyIlluminationRenderer owns all GPU resources. Terrain supplies its successfully emitted leaf IDs, final-heightmap slices and monotonically increasing generation stamps in the same order as body descriptors. These leaves have passed the existing frustum test; neither cached parents, warmup requests nor invisible leaves schedule illumination work. Parent descriptor layouts and terrain topology remain unchanged. The Controls / Sky illumination panel starts at four tile updates per frame (bounded 1–32), with 16 probes and 256 sky rays per probe: 16,384 rays per frame at N=4, before each ray's atmosphere/cloud integration.

Each tile's 4×4 probe lattice is cell-centered: fractions 0.125, 0.375, 0.625 and 0.875 on each axis, with half-pitch margins. Probe XZ and source placements subtract integer Position grid coordinates before float conversion. Each probe independently bilinearly samples the resident 259×259 final heightmap at 1 + fraction × 256 and sits 2 m above it. Receiver interpolation uses fraction × 4 − 0.5, clamped to the grid. Its 16×16 deterministic angular lattice samples equal solid angle over the upper hemisphere only, with staggered azimuth rings and midpoint elevations. No direct solar disk, ground bounce, terrain occlusion, moonlight, material exposure or artificial ambient floor is added. These are ground-level sky-irradiance probes, not full scene GI.

The compute shader shares CloudRenderer's once-prepared field and optics, the sky cubemap's hardware sRGB decode, disk-free linearSkyRadiance, and cloud_integration.glsl. Cloud radiance composites over clear sky from the probe's actual altitude. Rays cover the loaded cloud footprint/slab with the existing fullscreen distance-aware budget, independently of water's multiplier/distance and without camera-depth clipping. Disabled clouds still yield clear-sky irradiance. Nine RGB spherical-harmonic coefficients per probe store the hemisphere radiance projection with the Lambert convolution applied once. Terrain bilinearly interpolates the grid, evaluates the SH against the material normal and multiplies irradiance by albedo/π and the existing AO factor. Direct cloud shadows and caustics remain separate.

The manager uses a fixed 1024-slot asset cache; it protects every visible tile and retained ancestor before new admission. Up to N jobs refresh the oldest data, with missing/revised tiles treated as one refresh cycle old and distance breaking ties. This initializes nearby data while allowing retained tiles to refresh during continuous movement. New slots and changed height-generation stamps initialize directly; later updates use elapsed-time exponential blending with a 0.25 s time constant. Cached ancestor grids support subdivided children while pending. Where no ancestor exists, sampling temporarily falls back to the nearest available visible sky grid. This approximation is most noticeable during first fill, sudden travel and large elevation changes.

Frame-local receiver regions have smooth overlapping support one probe pitch beyond their tile boundaries. A bounded CPU pass builds conservative lists containing every support that can contribute within each receiver tile, including bridge edge roundoff. At the same position, adjacent receivers therefore use the same contributors/weights, including coarse/fine edges and corners. Inside each contributor, clamped bilinear interpolation remains continuous. Tile identity and generation stamps validate cached payloads; receiver indices are frame-local acceleration only. Body varying location 3 carries gl_InstanceIndex, while bridges carry their referenced parent index. The shader math and the read-only buffer interface are reusable: later renderers can call regionForTile for a receiver hint and select their own storage bindings via the GLSL binding macros. Foliage now borrows the same interface as described in section 5.4.

The renderer stores a persistent 2,359,296-byte SH buffer, a bounded frame-region/candidate buffer and one reusable upload buffer. Compute uses three samplers, one read-only heightmap buffer, one read/write SH buffer and a 1904-byte uniform block (cloud state at 0, sky rotation at 304, 32 × 48-byte jobs at 368). One 256-thread workgroup integrates a probe; 16 groups per scheduled tile reduce through 4 KiB shared incident radiance. Terrain adds two fragment storage buffers after its nine samplers, at bindings 9/10; frame data starts with a 16-byte header, 512 × 48-byte regions, then uint32 candidate indices. C++ assertions and compiled shader layouts must match.

Preparation precedes the early copy pass, cloud upload prepares the shared phases, and probe compute follows heightmap generation and precedes viewport draws. The persistent SH buffer never cycles: preceding reads, refresh writes and subsequent reads remain ordered on the same GPU command queue. Ready metadata means ready for subsequent commands after the mandatory dispatch, not CPU-readable completion. Failed command recording/submission aborts the frame through the existing error path. No asynchronous worker, CPU readback, fence wait or extra generation queue is needed for this queue-ordered field. Initialization clears manager residency when GPU storage is recreated; origin shifts and viewport resize preserve coefficients.

Validation: canonical Debug/Release builds and all six tests in each pass, including centered probe coordinates, hemisphere integration, scheduling fairness, height revisions, ancestor fallback, mixed-LOD contributor continuity, cache churn and large origins. All 80 staged SPIR-V modules validate. The 720-frame Debug cloud traversal completed all 12 stages and 24 macro revisions with 1,750 visible tile probe updates and no logged validation warnings/errors (`build/Debug/app/sky-illumination-traversal.log`). A Release startup frame also exited successfully.

Review N=4 GPU cost, sky/sun balance on snow, moving clouds, low sun, disabled clouds, transitions between terrain/cloud altitudes, camera/frustum/LOD changes and cell/universe-origin crossings. Sparse angular samples can miss narrow sky features; coarse probe grids approximate changing ground altitude and local cloud variation. Scheduling and temporal blending introduce lighting lag. The existing artistic sky/cloud source scales are reused without compensation, so this does not establish a physical radiometric calibration or guarantee a particular displayed snow brightness.
