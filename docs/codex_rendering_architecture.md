...
Water environment audit: skybox and water use the same `AtmosphereOptics`, sky display settings, inverse sky rotation, cubemap sampler, atmosphere height and base sky distance, and `displaySkyRadiance`. Water starts atmosphere integration at its displaced surface altitude, correctly excluding the camera-to-surface segment. The only identified sampling discrepancy was the missing sky-distance extension for downward rays starting above the atmosphere; water now matches the skybox there. This does not explain a sea-level daytime yellow/dark appearance. Reflection strength remains 1.0; Fresnel, roughness, distance-flattened reflection normals, reflection mixing, body lighting, foam, and overhead blue styling are unchanged. Appearance tuning remains for user review.

Regular and bridge water borrow coverage/noise sampler bindings and a 256-byte prepared cloud sampling block from `CloudRenderer`. `upload()` prepares phases and common lighting once inside the early frame copy pass; the later fullscreen pass consumes that same state with its separate inverse view-projection matrix. `cloud_integration.glsl` integrates an explicit render-relative origin, direction and distance limit. Water uses the displaced surface and slab/max-distance clipping for topside reflection or non-TIR underside transmission; only the fullscreen caller clips against scene depth. Linear cloud radiance accumulates with air transport from the ray origin, maps its effective source once, and composites over displayed clear sky. Disabled/unready sampling skips the march while retaining bindings to the renderer-owned textures. Water ambientSky remains clear-sky-only, and cloud ambient uses its own unchanged setting, including zero.
...
`cloud_density.glsl` is camera independent: its inputs are a position relative to the shared render origin, slab/coverage parameters, the two textures, and bounded base/detail phases. CPU `PeriodicWorldPhase` handles integer world cells before float conversion. Wind advances wrapped phase values using a double-precision monotonic clock; no unbounded shader time or absolute world XZ is reconstructed. Frequencies in the settings panel are cycles/km (user-tuned defaults 0.05 and 0.1), converted to cycles/m in uniforms. Density applies the vertical profile, coverage, base remapping, then detail erosion sequentially. The cloud slab is horizontal and fixed in world altitude.

The dedicated fullscreen pass follows the unchanged sky/medium compositor. It intersects depth-reconstructed view rays with the slab and scene depth, then uses bounded fixed-step marching and early transmittance termination. Each occupied view sample performs one sun march to the actual slab exit. That optical depth feeds every dual-HG scattering octave and the view-dependent powder multiplier; there is no additional Beer multiplier over the octave sum. Camera direction points from camera into scene, sun direction points toward the sun: mu=-1 sees the sun-facing cloud side, mu=+1 is transmitted/backlit. Ambient uses the atmospheric solar tint. Linear source radiance, including atmosphere along the camera path, accumulates with analytic per-step transmittance, then maps once into the existing display-referred target and composites with premultiplied opacity. This retains the existing bounded sky display approximation; it does not introduce HDR exposure or redesign the atmosphere.

Settings are exposed in Controls / Clouds, with texture dimensions configured at startup. `cloud_tests` compiles actual GLSL noise/optical/slab math on CPU and checks periodicity, cache churn, borders, seed invalidation, universe-scale phases, slab intersections, scattering limits, and analytic integration. `--verify-clouds --quit-after-frames 720 --disable-steam` exercises GPU traversal across both slab boundaries, horizontal rays, grid transitions, seed changes, and cells at 2^55. The default marcher is deliberately straightforward: quality/cost are controlled by view and sun sample counts. SDL 3.4's Vulkan backend can emit a 3D-image barrier forward-compatibility warning concerning maintenance9; the application does not enable that feature or control SDL's barrier subresource ranges.
