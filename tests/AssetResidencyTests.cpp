...
struct Fence { bool signaled = false; [[nodiscard]] bool isSignaled() const { return signaled; } };
struct Job { int assetId = 0; std::uint16_t targetSlot = 0; };
...
struct TestManager
{
    ...
    {
        auto slot = hint != unavailable && cache.validatesHint(hint, assetId)
...
int main()
{
    ...
    cache.assign(0, 10); cache.markReady(0);
    cache.assign(1, 20); cache.markReady(1);
    ...
    for (int i = 0; i < 300; ++i) cache.age();
    assert(cache.ageOf(0) == 255);
    cache.touch(0); assert(cache.ageOf(0) == 0);
    ...
    cache.clearActiveJob(0, b); assert(cache.activeJob(0) == a);
    cache.clearActiveJob(0, a); assert(!cache.activeJob(0).valid());
    ...
    assert(clearQueue.count() == 2); // Submitted front remains; unsubmitted work is semantically complete behind it.
