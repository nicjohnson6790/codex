...
- Fast LZ4 can exceed the 128 KiB tile staging bound. Offline heightmap compression retries oversized output with LZ4-HC; `converter.exe heightmap-repack <assetbin>` losslessly migrates existing v3 packs without re-quantization. The 37 oversized local ETOPO/DEM10 CE/NE tiles were migrated; `.assetbin.before-bounded` backups retain the original indices and original blobs remain in the data files.
