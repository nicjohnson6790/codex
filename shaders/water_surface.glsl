layout(set=WATER_UNIFORM_SET, binding=WATER_UNIFORM_BINDING) uniform WaterUniforms
{
    mat4 viewProjection;
    vec4 cameraAndTime;
    vec4 waterParams;
...
void evaluateWaterSurface(WaterInstance instance, vec2 localMeters, out vec3 position,
    out vec3 displacement, out float viewDistance, out float localDepth, out float shoreFactor)
{
    ...
    viewDistance = length(position);
    float metersPerPixelAtView = metersPerPixel(viewDistance);

    localDepth = water.depthEffectParams.x;
    if (hasTerrainSlice)
    {
